...
public class CigaretteSmoke : MonoBehaviour
{
	...
	void PumpFluid(){
		...
		int spotHalfWidth = 2;
		for (int spotJ = pumpPosIntY - spotHalfWidth; spotJ < pumpPosIntY + spotHalfWidth; spotJ++) {
			for (int spotI = pumpPosIntX - spotHalfWidth; spotI < pumpPosIntX + spotHalfWidth; spotI++) {
				if (spotJ >= 0 && spotJ < height && spotI >= 0 && spotI < width) {
					int idx = spotJ * numRows + spotI + 1;
					prev_density [idx] += pumpDensity;
				}
			}
		}
